...
class DataAnalysisAgent(BaseAgent):
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            ...
            {"error": "error_handling", "continue": "schema_retrieval"},
        )

        workflow.add_conditional_edges(
            "sql_generation",
            self._should_handle_error,
            {"error": "error_handling", "continue": "query_execution"},
        )
    ...
    async def _understand_intent(self, state: AnalysisState) -> AnalysisState:
        ...
        try:
            ...
            {{
                "intent_type": "query|dashboard|exploration",
        ...
        try:
            ...
            for entity in intent.entities:
                try:
                    ...
                    desc_schemas = (
                        await self.schema_manager.search_tables_by_description(entity)
                    )
                    relevant_schemas.extend(
                        [schema.to_dict() for schema in desc_schemas]
                    )

                except Exception as e:
                    self.logger.warning(
                        f"Schema search failed for entity {entity}", error=str(e)
                    )
    ...
    async def _generate_sql(self, state: AnalysisState) -> AnalysisState:
        ...
        try:
            ...
            state["sql_explanation"] = (
                f"Generated query to analyze {intent.intent_type}"
            )
    ...
    async def _execute_query(self, state: AnalysisState) -> AnalysisState:
        ...
        try:
            ...
            df = await self.bigquery_client.execute_query(
                query=sql_query, timeout=60.0, max_results=1000
            )
    ...
    async def _handle_error(self, state: AnalysisState) -> AnalysisState:
        ...
        state["final_response"] = (
            f"I encountered an error while processing your request: {error_msg}"
        )
    ...
    def _parse_intent_basic(self, query: str) -> AnalysisIntent:
        ...
        if any(word in query_lower for word in ["dashboard", "report"]):
    ...
    def _generate_sql_basic(
        self, intent: AnalysisIntent, schemas: List[Dict[str, Any]]
    ) -> str:
    ...
    async def run(
        self, user_query: str, user_id: str, session_id: str
    ) -> Dict[str, Any]:
    ...
    async def astream(self, **kwargs: Any) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        initial_state = AnalysisState(
            ...
            current_step="",
            error_count=0,
            messages=[HumanMessage(content=kwargs.get("user_query"))],
            metadata={"start_time": datetime.utcnow().isoformat()},
        )
